...
func writeWorkItemOutput(outDir string, content workItemContent, rawWorkItem, rawComments json.RawMessage) error {
	if err := writeRawJSON(filepath.Join(outDir, "raw", "work-item.json"), rawWorkItem); err != nil {
		return err
	}
	if len(rawComments) > 0 {
		if err := writeRawJSON(filepath.Join(outDir, "raw", "comments.json"), rawComments); err != nil {
			return err
		}
	}
	...
	return writeWorkItemMarkdown(filepath.Join(outDir, "content.md"), content)
}
func writeWorkItemMarkdown(path string, c workItemContent) error {
	var b strings.Builder
	fmt.Fprintf(&b, "# ADO Work Item %s: %s\n\n", c.ID, firstNonEmpty(c.Fields.Title, "(untitled)"))
	...
	if len(c.Comments) == 0 {
		b.WriteString("No comments fetched.\n")
	} else {
		for _, cm := range c.Comments {
			fmt.Fprintf(&b, "- %s by %s\n\n%s\n\n", cm.CreatedDate, firstNonEmpty(cm.CreatedBy, "(unknown)"), firstNonEmpty(cm.Text, "(empty)"))
		}
	}
	fmt.Fprintf(&b, "## Links\n\n- Parents: %s\n- Children: %s\n- Related: %s\n\n## Attachments\n\n", strings.Join(c.Relations.ParentIDs, ", "), strings.Join(c.Relations.ChildIDs, ", "), strings.Join(c.Relations.RelatedIDs, ", "))
	if len(c.Attachments) == 0 {
		b.WriteString("No attachments.\n")
	} else {
		for _, att := range c.Attachments {
			fmt.Fprintf(&b, "- %s", att.OriginalFilename)
			if att.Status != "" {
				fmt.Fprintf(&b, " (%s)", att.Status)
			}
			if att.AssetPath != "" {
				fmt.Fprintf(&b, " `%s`", att.AssetPath)
			}
			if att.MarkdownPath != "" {
				fmt.Fprintf(&b, " markdown `%s`", att.MarkdownPath)
			}
			b.WriteByte('\n')
		}
	}
...
func writeTreeOutput(outDir string, content treeContent) error {
	...
	var b strings.Builder
	fmt.Fprintf(&b, "# ADO Work Item Tree %s\n\n- Nodes fetched: %d\n- Edges discovered: %d\n- Max depth: %d\n- Max items: %d\n\n", content.RootID, len(content.Nodes), len(content.Edges), content.MaxDepth, content.MaxItems)
	for _, n := range content.Nodes {
		fmt.Fprintf(&b, "- %s depth %d: %s [%s] `%s`\n", n.ID, n.Depth, firstNonEmpty(n.Fields.Title, "(untitled)"), n.Fields.State, n.ItemPath)
	}
	...
	return writeFile(filepath.Join(outDir, "content.md"), []byte(b.String()))
}
