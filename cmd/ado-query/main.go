...
func runWorkItem(args []string) error {
	opts, err := parseQueryFlags("work-item", args, false)
	if err != nil {
		return err
	}
...
func runTree(args []string) error {
	opts, err := parseQueryFlags("work-item-tree", args, true)
	if err != nil {
		return err
	}
	opts, err = normalizeOptions(opts)
...
func parseQueryFlags(name string, args []string, tree bool) (queryOptions, error) {
	...
	fs.BoolVar(&opts.noCache, "no-cache", false, "Bypass cache")
	fs.StringVar(&opts.apiVersion, "api-version", defaultAPIVersion, "Azure DevOps API version")
	fs.BoolVar(&opts.includeAttachments, "include-attachments", false, "Download attachments")
	fs.Int64Var(&opts.maxAttachmentBytes, "max-attachment-bytes", defaultMaxAttachmentBytes, "Maximum attachment bytes")
	if tree {
		fs.IntVar(&opts.maxDepth, "max-depth", defaultMaxTreeDepth, "Maximum tree depth")
		fs.IntVar(&opts.maxItems, "max-items", defaultMaxTreeItems, "Maximum work items")
	}
	flagArgs, id, err := splitArgs(args, map[string]bool{"org": true, "project": true, "out": true, "cache-dir": true, "api-version": true, "max-attachment-bytes": true, "max-depth": true, "max-items": true})
	if err != nil {
		return opts, err
	}
	...
	if id == "" {
		return opts, fmt.Errorf("%s requires exactly one id", name)
	}
	if opts.maxAttachmentBytes <= 0 {
		return opts, errors.New("--max-attachment-bytes must be positive")
	}
	if tree && (opts.maxDepth < 0 || opts.maxItems <= 0) {
...
Auth uses Azure CLI Microsoft Entra credentials:
  az login
