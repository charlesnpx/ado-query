...
Run `ado-query work-item <id>` for one item or `ado-query work-item-tree <id>` for descendants. Always run the CLI for current context instead of trusting an old output directory directly; cached work items, comments, and included attachments are validated on every run. By default, output is materialized under `~/.cache/ado-query/outputs/...`; pass `--out <dir>` only when caller-local files are required. Read `content.md` first, then `content.json` and raw payloads only when needed. Use `--include-attachments` when the task requires attached documents or images, and report any stale or missing attachment status from the output warnings.
